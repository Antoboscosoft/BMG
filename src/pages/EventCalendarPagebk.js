...
function EventCalendarPage({ navigation }) {
    ...
    const fetchEvents = async () => {
        try {
            ...
            Toast.show(
                languageTexts?.eventCalendar?.error?.fetchEvents || 'Failed to fetch events',
                Toast.LONG
            );
        } finally {
            // setLoading(false);
    ...
    const handleRegister = (event) => {
        try {
            ...
            navigation.navigate('CreateRegister', {
                eventData: {
                    id: event.id,
                    title: languageTexts?.eventCalendar?.events?.[event.titleKey] || event.title,
                    description: languageTexts?.eventCalendar?.descriptions?.[event.descriptionKey] || event.description,
                    location: languageTexts?.eventCalendar?.locations?.[event.locationKey] || event.location,
                    isRegistered: false
                }
            });
        } catch (error) {
            console.error('Registration Error:', error);
            Toast.show(languageTexts?.eventCalendar?.error?.register || 'Failed to start registration. Please try again.', Toast.LONG);
        }
    };
    ...
    const handleRegistered = (event) => {
        try {
            ...
            Toast.show(languageTexts?.eventCalendar?.error?.registered || 'Failed to view registration. Please try again.', Toast.LONG);
        }
    };
        ...
        <LinearGradient colors={['#2753b2', '#e6e9f0']} style={styles.container}>
            <Animated.View style={[styles.innerContainer, { opacity: fadeAnim }]}>
                <View style={styles.headerRow}>
                    ...
                    <Text style={styles.titleText}>{languageTexts?.menu?.eventCalendar || 'Event Calendar'}</Text>
                    <View style={{ width: 60 }} />
                </View>

                <ScrollView contentContainerStyle={styles.contentContainer} showsVerticalScrollIndicator={false}>
                    <Text style={styles.subtitleText}>{languageTexts?.eventCalendar?.subtitle || 'View upcoming events and activities.'}</Text>
                    ...
                    <View style={styles.eventsContainer}>
                        {selectedEvents.length > 0 ? (
                            selectedEvents.map((event) => (
                                <View key={event?.id} style={styles.eventCard}>
                                    <View style={styles.eventContent}>
                                        <Text style={styles.eventTitle}>{languageTexts?.eventCalendar?.events?.[event.titleKey] || event?.title}</Text>
                                        ...
                                        <View style={styles.eventDetailRow}>
                                            ...
                                            <Text style={styles.eventDetailText}>{languageTexts?.eventCalendar?.locations?.[event.locationKey] || event?.location}</Text>
                                        </View>

                                        <View style={styles.eventDetailRow}>
                                            <Icon name="notes" size={20} color="#2753b2" style={styles.icon} />
                                            <Text style={styles.eventDescription}>{languageTexts?.eventCalendar?.descriptions?.[event.descriptionKey] || event?.description}</Text>
                                        </View>
                                    <View style={styles.buttonContainer}>
                                        {event.registered === null ? (
                                            ...
                                            >
                                                <Text style={styles.registerButtonText}>{languageTexts?.eventCalendar?.register || 'Register'}</Text>
                                            </TouchableOpacity>
                                        ) : (
                                            <TouchableOpacity
                                            ...
                                            >
                                                <Text style={styles.registeredButtonText}>{languageTexts?.eventCalendar?.registered || 'Registered'}</Text>
                                            </TouchableOpacity>
                                        )}
                                    </View>
                                    </View>
                            ...
                            <View style={styles.noEventsCard}>
                                <Text style={styles.noEventsText}>{languageTexts?.eventCalendar?.noEvents || 'No events scheduled for this date'}</Text>
                            </View>
                        )}
                    </View>
