...
import { Animated, Dimensions, StyleSheet, Text, TouchableOpacity, View, TouchableWithoutFeedback, PanResponder, Image, FlatList, ActivityIndicator, Modal, BackHandler, Pressable, StatusBar, PermissionsAndroid, } from 'react-native';
import LinearGradient from 'react-native-linear-gradient';
import Icon from 'react-native-vector-icons/MaterialCommunityIcons';
import Feather from 'react-native-vector-icons/Feather';
import { SafeAreaView } from 'react-native-safe-area-context';
import { getUserData, sendUserLocation, updateFirebaseToken } from '../api/auth';
...
function DashboardPage({ navigation, route }) {
    ...
    const checkAndUpdateLocation = async () => {
        try {
            ...
            const timeDiffHours = (currentTime - lastSyncTime) / (1000 * 60 * 60); // difference in hours from last sync
            console.log('[Dashboard] Time since last sync (hrs):', timeDiffHours);

            if (timeDiffHours >= everyTimeSendLocationtoBackendTime) {
                ...
                console.log(`[Dashboard] Less than {${everyTimeSendLocationtoBackendTime}} hours since last sync. No need to update location`);
            }
