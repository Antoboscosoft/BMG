  ...
  return (
    ...
    >
      <LinearGradient colors={['#5e3b15', '#b06a2c']} style={styles.background}>
        ...
        <View style={[styles.floatingButtonContainer, { justifyContent: locaionShow ? 'space-between' : 'flex-end' } ]}>
          {locaionShow && (
            <TouchableOpacity
              style={[styles.floatingButton, styles.locationHistoryButton]}
              onPress={() => navigation.navigate('LocationHistory', { userData, from: route.params?.from || 'Profile' })}
            >
              <Icon name="location-on" size={24} color="#3D2A1A" />
              <Text style={styles.floatingButtonText}>{languageTexts?.profile?.locationView?.title || "Location History"}</Text>
            </TouchableOpacity>
          )}
          <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  locationHistoryButton: {
    ...
    minWidth: 140, // Minimum width to ensure text fits
  },
  editButton: {
    // No additional styles needed, keeps icon-only
  },
  floatingButtonText: {
    fontSize: 16,
    color: '#3D2A1A',
